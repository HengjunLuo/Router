...
public class RouterSimulator {
	...
	***************** NETWORK EMULATION CODE STARTS BELOW ***********
	...
	should not have to, and you definitely should not have to modify
	******************************************************************/
	...
	void toLayer2(RouterPacket packet) {
		...
		packetptr = (RouterPacket)packet.clone();
		
		if (TRACE > 2) {
			myGUI.print("    TOLAYER2: source: " + packetptr.sourceid + " dest: " + packetptr.destid + "             costs:");
			
			for (i=0; i<NUM_NODES; i++)
				myGUI.print(packetptr.mincost[i] + " ");
		...
		eventptr = new Event();
		eventptr.evtype =  FROM_LAYER2;   /* packet will pop out from layer3 */
		eventptr.eventity = packet.destid; /* event occurs at other entity */
		eventptr.rtpktptr = packetptr;       /* save ptr to my copy of packet */
		...
		lastime = clocktime;
		for (q = evlist; q != null ; q = q.next)
			if ((q.evtype == FROM_LAYER2  && q.eventity == eventptr.eventity) ) 
				lastime = q.evtime;
		
		eventptr.evtime =  lastime + 9 * generator.nextDouble() + 1;
...
class  Event {
	...
	RouterPacket rtpktptr;	/* ptr to packet (if any) assoc with this event */
